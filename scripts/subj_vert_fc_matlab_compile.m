function subj_vert_fc(fc_configfp)
	...
	outdir=passed_ml_args.outdir;
	outdirp=passed_ml_args.outdirp;
	...
	partcoefneg=zeros(17734,length(Krange));
        ...
        vw_ts_l_p=['/cbica/projects/pinesParcels/data_psy/CombinedData/' num2str(subjs(s)) '/lh.fs5.sm6.residualised.mgh'];
	vw_ts_r_p=['/cbica/projects/pinesParcels/data_psy/CombinedData/' num2str(subjs(s)) '/rh.fs5.sm6.residualised.mgh'];
	...
	for K=2:max(Krange)
		...
		Kmat=zeros(K);
                ...
                bTS_Kmat=zeros(K);
		% load in partitions
		K_Folder = ['/cbica/projects/pinesParcels/data_psy/SingleParcellation/SingleParcel_1by1_kequal_' num2str(K) '/Sub_' num2str(subjs(s))];
